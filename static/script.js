            ...
            calculateAllBtn.addEventListener('click', () => {
                try {
                    ...
                    const results = {
                        ...
                        division: { 
                            dec: n2 !== 0n ? (n1 / n2) : null, 
                            op: '÷', 
                            rem: n2 !== 0n ? (n1 % n2) : null 
                        }
                    };
        ...
        function displayAllOpsResults(num1, num2, data) {
            ...
            opsResultsGrid.innerHTML = Object.entries(data).map(([opName, res]) => {
                ...
                if (opName === 'division') {
                    ...
                    else if (res.rem !== 0n) bijectiveResult = `${toBijective(res.dec)} (Rem: ${toBijective(res.rem)})`;
                    else bijectiveResult = toBijective(res.dec);
                } else {
                    bijectiveResult = toBijective(res.dec);
                }

                const decimalDisplay = res.dec !== null ? res.dec.toString() : 'N/A';
